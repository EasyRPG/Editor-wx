...
{int number;
    	CRpgIOBase		st;
    	st.OpenFromMemory((Get(CDim1(col))).GetConstPtr(), (Get(CDim1(col))).GetSize());
    return (st.ReadBerNumber());	
   
    
    /*if(IsExist(CDim1(col)))
    {
    return( CRpgUtil::GetBerNumber((smart_buffer &) *(Get(CDim1(col)))  ));
    printf("existo");
    }
    else
   {     printf("no existo");
...
{
	return (IsExist(CDim1(col))) ? (CRpgUtil::GetBerNumber( (smart_buffer &) *(Get(CDim1(col)))  ) != 0 ) : def;
}
...
{
    CRpgIOBase		st;
    	st.OpenFromMemory(( Get(CDim2(col,row))).GetConstPtr(), ( Get(CDim2(col,row))).GetSize());
    return (st.ReadBerNumber());	
   
	//return (IsExist(CDim2(col,row))) ? CRpgUtil::GetBerNumber((smart_buffer &) *( Get(CDim2(col,row)))) : def;
}
...
{
	return (IsExist(CDim2(col,row))) ? (CRpgUtil::GetBerNumber( (smart_buffer &) *(Get(CDim2(col,row))) ) != 0 ) : def;
}
