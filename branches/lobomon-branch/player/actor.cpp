...
//extern CWorld  *World; //when this exists
#include "SDL.h"
#include "math-sll.h"
#include "sprite.h"
#include "actor.h"
#include "deltatime.h"
extern Delta_time My_system_time;
...
// 	    grid_x=(x_pos)>>4; grid_y=(y_pos)>>4;// Calculate Grid x_pos and Grid y_pos
    ...
    case ACTOR_STATE_MOVING:
        ...
        case ACTOR_DIRECTION_UP:
            real_y = sllsub(real_y, Motion.delta);
            break;
        case ACTOR_DIRECTION_DOWN:
            real_y = slladd(real_y, Motion.delta);
            break;
        case ACTOR_DIRECTION_LEFT:
            real_x = sllsub(real_x, Motion.delta);
            break;
        case ACTOR_DIRECTION_RIGHT:
            real_x = slladd(real_x, Motion.delta);
            break;
        }

        if (Motion.distance == 16.0f)
        {
