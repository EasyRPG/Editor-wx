...
std::vector <Magicblock> LDB::heroskillChunk(FILE * Stream)
{
...
std:: vector <stcSkill> LDB::skillChunk(FILE * Stream)
{		
		 int id,datatoread=0,datareaded=0; 
...
std:: vector <stcItem> LDB::itemChunk(FILE * Stream)
{		
		 int id,datatoread=0,datareaded=0; 
		 std::string name;
...
std:: vector <stcEnemy_Action> LDB::mosteractionChunk(FILE * Stream)
{ 
 int id,datatoread=0,datareaded=0;
...
std:: vector <stcEnemy> LDB::mosterChunk(FILE * Stream)
{		
		 int id,datatoread=0,datareaded=0; 
...
std:: vector <stcEnemy_group_data> LDB::MonsterPartyMonsterChunk(FILE * Stream)
{ 
 int id,datatoread=0,datareaded=0;
...
stcEnemy_group_condition  LDB::MonsterPartyEventconditionChunk(FILE * Stream)
{ 
...
std:: vector <stcEnemy_group_event_page> LDB::MonsterPartyevent_pageChunk(FILE * Stream)
{ 
 int id,datatoread=0,datareaded=0;
...
std:: vector <stcEnemy_group> LDB::mosterpartyChunk(FILE * Stream)
{		
		 int id,datatoread=0,datareaded=0; 
...
std:: vector <stcTerrain> LDB:: TerrainChunk(FILE * Stream)
{		
		 int id,datatoread=0,datareaded=0; 
		 std::string name;
...
std:: vector <stcAttribute> LDB:: AttributeChunk(FILE * Stream)
{		
		 int id,datatoread=0,datareaded=0; 
		 std::string name;
...
std:: vector <stcState> LDB:: StatesChunk(FILE * Stream)
{		
		 int id,datatoread=0,datareaded=0; 
		 std::string name;
...
std:: vector <stcAnimationTiming> LDB:: AnimationTimingChunk(FILE * Stream)
{		
		 int id,datatoread=0,datareaded=0; 
...
std:: vector <stcAnimationCelldata> LDB:: AnimationCelldataChunk(FILE * Stream)
{		
		 int id,datatoread=0,datareaded=0; 
...
std:: vector <stcAnimationCell> LDB:: FramedataChunk(FILE * Stream)
{		
		 int id,datatoread=0,datareaded=0; 
...
std:: vector <stcAnimated_battle> LDB:: AnimationChunk(FILE * Stream)
{		
		 int id,datatoread=0,datareaded=0; 
...
std:: vector <stcChipSet> LDB:: TilesetChunk(FILE * Stream)
{		
		 int id,datatoread=0,datareaded=0; 
		 short dat=0;
		 stcChipSet ChipSet;
...
std:: vector <stcEvent> LDB:: EventChunk(FILE * Stream)
{		
		 int id,datatoread=0,datareaded=0; 
...
stcGlosary LDB::StringChunk(FILE * Stream)//movimientos de la pagina
{		
	stcGlosary Glosary;  
	do{ 
		ChunkInfo.ID	 = ReadCompressedInteger(Stream); // lectura de tipo del pedazo	 
...
stcMusic_Background  LDB::musicChunk(FILE * Stream)
{ 
...
std::vector <stcBattle_test>  LDB::Batletest(FILE * Stream)
{
int id,datatoread=0,datareaded=0; 
...
stcSystem LDB::SystemChunk(FILE * Stream)//movimientos de la pagina
{		 short dat;
		 stcSystem System; 
			  do{			   
				ChunkInfo.ID	 = ReadCompressedInteger(Stream); // lectura de tipo del pedazo	 
...
std:: vector <std::string> LDB::Switch_VariableChunk(FILE * Stream)//simplemente un vector de string
{		
		 int id,datatoread=0,datareaded=0; 
		 std::string name;
...
std:: vector <stcCombatcommand> LDB:: Comand_Chunk2(FILE * Stream)
{		
		 int id,datatoread=0,datareaded=0; 
...
stcCombatcommands LDB:: Comand_Chunk(FILE * Stream)//todo leido
{		
...
std:: vector <stcProfetion>  LDB:: Profession_Chunk(FILE * Stream)
{		
...
std:: vector <stcFight_anim> LDB:: Fightanim_Chunk2(FILE * Stream)
{		
		 int id,datatoread=0,datareaded=0; 
...
std:: vector <stcBattle_comand> LDB:: Fightanim_Chunk(FILE * Stream)
{		
		 int id,datatoread=0,datareaded=0; 
		...
		while(!feof(Stream))
		{
		 ...
		 if(ChunkInfo.Length>0)  
			...
			{																										   
			 ...
			 case CHUNK_Hero:
					heros= heroChunk(Stream);
				 break;
			 case CHUNK_Skill:
					skill= skillChunk(Stream);
					break; 
			 case CHUNK_Item:
					items=itemChunk(Stream);
					break;  
			case CHUNK_Monster:
					mosters= mosterChunk(Stream);
					break;
			case CHUNK_MonsterP:
					mosterpartys=mosterpartyChunk(Stream);
					break;  
			case CHUNK_Terrain:
					Terrains=TerrainChunk(Stream);
					break;	   
			case CHUNK_Attribute:
				        Attributes=AttributeChunk(Stream);
					break;  
			case CHUNK_States:
				        States=StatesChunk(Stream);
					break; 		
			case CHUNK_Animation:
				        Animations=AnimationChunk(Stream);
					break;  
			case CHUNK_Tileset:
				        Tilesets=TilesetChunk(Stream);
					break; 	  
			case CHUNK_String:
				        Glosary=StringChunk(Stream);
					break;	
			case CHUNK_System:	
				        System_dat=SystemChunk(Stream);
					break;			
			case CHUNK_Event: 
				        Event=EventChunk(Stream);
					break;    
			case CHUNK_Switch:
				        Switch_Names= Switch_VariableChunk(Stream);
					break;
			case CHUNK_Variable: 
				        Variable_Names =Switch_VariableChunk(Stream);
					break;		
			...
			case CHUNK_Event3://no existe
				...
				printf("%d",Void);
					...
					break;	
		        case CHUNK_Comand:
			     Combatcommands=Comand_Chunk(Stream);
			     break; 
			case CHUNK_Profession:
			     Professions=Profession_Chunk(Stream);
			     break;  
			...
			case CHUNK_Fightanim:
			     Fightanims=Fightanim_Chunk(Stream);
					break; 
